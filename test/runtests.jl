...
Z = sm_score*V;
@test all( lre( attention(Q,K,V, Float32(1./sqrt(d_attn))) - score) .> 10)
...
@test_broken encoder_params == 4 + mha_params + 2*2 # current implementation has 2 extra parameters for a final layernorm (In annotated transformer bubut not paper)
...
@test encoder_stack_params == 6*encoder_params
...
@test_broken length(ps) == 4 + 6 + 6 + 2*2 + 3*2 # we implemented like the annotated transformer, with an extra final layernorm 
