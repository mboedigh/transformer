...
model = Transformer(max_seqlen=max_seqlen, d_vocab=d_vocab, d_model=d_model, p_drop = p_drop, n_layers = n_layers, n_heads = n_heads);

n_batches = 20;
batch_size = 30;
dataset = data_gen( batch_size, d_vocab, max_seqlen, d_model, n_batches);
